...
export const PostListItem: FC<Props> = ({ post }) => {
  ...
  return (
    <div
      className='flex cursor-pointer items-center gap-5 rounded bg-orange-50 px-5 py-3 shadow transition-transform hover:scale-105'
      onClick={() => router.push(`/posts/${post.id}`)}
    >
      <div className='flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full bg-white shadow'>
        <div className='pb-1 text-3xl'>{meta.icon}</div>
      </div>
      <div className='w-full space-y-3'>
        <div className='flex justify-between gap-3'>
          <h3 className='font-bold'>{meta.title}</h3>
          <p className='whitespace-nowrap text-sm'>{meta.date}</p>
        </div>
